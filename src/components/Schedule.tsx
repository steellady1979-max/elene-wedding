        ...
        <Reveal>
          <div className="relative overflow-hidden rounded-none border-x-0 border-t-0 border-b border-ink/10 bg-parchment/95 py-5 shadow-none sm:rounded-2xl sm:border sm:p-7 sm:shadow-soft">
            <div className="relative px-5 sm:px-0">
              <SparkleTitle className="font-geo text-lg tracking-[0.15em]">
                დღის განრიგი
              </SparkleTitle>
            </div>
            <ol className="mt-6 grid gap-5">
              {ITEMS.map(
                ({ time, icon: Icon, title, place, map, image, alt, width, height }, i) => (
                    ...
                    <div className="min-w-0 flex-1 pb-1 pr-5 sm:pr-0">
                      ...
                      {image && (
                        <Reveal
                          delay={120}
                          duration={1200}
                          className="mt-4 flex justify-center -ml-[3.25rem] sm:ml-0"
                        >
                          <img
                            src={image}
                            width={width}
